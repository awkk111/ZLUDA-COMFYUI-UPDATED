...
PRIORITIZE_FP16 = True  # TODO: remove and replace with something that shows exactly which dtype is faster than the other
try:
...
if WINDOWS:
    EXTRA_RESERVED_VRAM = 200 * 1024 * 1024 #Windows is higher because of the shared vram issue
    if total_vram > (16 * 1024):  # more extra reserved vram on 16GB+ cards
...
def load_models_gpu(models, memory_required=0, force_patch_weights=False, minimum_memory_required=None, force_full_load=False):
    ...
    for device in total_memory_required:
        if device != torch.device("cpu"):
            free_memory(total_memory_required[device] * 1.0 + extra_mem, device)
...
def maximum_vram_for_weights(device=None):
    return (get_total_memory(device) * 0.40 - minimum_inference_memory())
...
def text_encoder_initial_device(load_device, offload_device, model_size=0):
    ...
    if mem_l > (mem_o * 0.7) and model_size * 1.2 < mem_l:
        return load_device
    else:
        return offload_device
...
if not args.disable_pinned_memory:
    if is_nvidia() or is_amd():
        if WINDOWS:
            MAX_PINNED_MEMORY = get_total_memory(torch.device("cpu")) * 0.25  # Windows limit is apparently 50%
        else:
            MAX_PINNED_MEMORY = get_total_memory(torch.device("cpu")) * 0.45
...
def should_use_fp16(device=None, model_params=0, prioritize_performance=True, manual_cast=False):
    ...
    if manual_cast:
        free_model_memory = maximum_vram_for_weights(device)
        if (not prioritize_performance) or model_params * 2 > free_model_memory:
            return True
